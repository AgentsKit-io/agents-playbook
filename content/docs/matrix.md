...
| **Quality** | Define "done" per surface (completeness contract); agent success metrics + rubric | Test pyramid; coverage targets per package; eval-set design | File-size budgets; lint rules; hermetic tests; deterministic + LLM-as-judge evals | `check:all`, `sanity`, structural gates, e2e; no-eval-no-merge | Pre-push hooks; release gates | Bug-hunt cadence; mutation testing; production agent monitoring |
