...
| **AI-collaboration** | CLAUDE.md, AGENTS.md, MEMORY.md bootstrap | Slash commands; sub-agent recipes; system prompts | Goal-mode loop; one-sub-unit-per-session rule | Verify-first close; duplication-claims-API-grounded | Phased PR + admin merge; self-describe manifest for external agents | Persistent memory; lessons graph |
