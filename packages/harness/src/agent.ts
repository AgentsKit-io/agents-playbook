...
type SessionEventType = 'session.started' | 'agent.turn.started' | 'tool.requested' | 'tool.completed' | 'tool.failed' | 'session.ended'
...
export const createSessionRecorder = ({ stateDir, run, adapter, sessionId = randomUUID() }: AgentSessionOptions): SessionRecorder => {
  ...
  const recorder: SessionRecorder = {
    ...
    requestTool: (input) => { open(); const turnId = required(input.turnId, 'turnId'); if (!turns.has(turnId)) fail(`Turn does not exist: ${turnId}.`, 'INVALID_STATE'); const actionId = required(input.actionId ?? randomUUID(), 'actionId'); if (pending.has(actionId)) fail(`Tool action already exists: ${actionId}.`, 'INVALID_STATE'); const event = append('tool.requested', { turnId, actionId, toolId: required(input.toolId, 'toolId'), argumentsHash: required(input.argumentsHash, 'argumentsHash') }); pending.add(actionId); return event },
