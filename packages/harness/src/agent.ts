...
type SessionEventType = 'session.started' | 'session.resumed' | 'agent.turn.started' | 'policy.evaluated' | 'tool.approval.requested' | 'tool.approval.recorded' | 'tool.requested' | 'tool.blocked' | 'tool.completed' | 'tool.failed' | 'session.ended'
...
export const createSessionRecorder = ({ stateDir, run, adapter, policy, runtime, sessionId = randomUUID(), resume = false }: AgentSessionOptions): SessionRecorder => {
  ...
  const released = new Map<string, { readonly turnId: string; readonly toolId: string; readonly argumentsHash: string }>()
  ...
  if (resume) {
    ...
    for (const event of prior) {
      ...
      if (event.type === 'tool.approval.recorded') { approvals.delete(event.payload.actionId); if (event.payload.decision === 'approved') released.set(event.payload.actionId, { turnId: event.payload.turnId, toolId: event.payload.toolId, argumentsHash: event.payload.argumentsHash }) }
      if (event.type === 'tool.requested') { actions.add(event.payload.actionId); pending.set(event.payload.actionId, { turnId: event.payload.turnId, toolId: event.payload.toolId, argumentsHash: event.payload.argumentsHash }); released.delete(event.payload.actionId) }
