...
program.command('approve <decision> [run-id]').description('Record human approval or rejection.').option('--by <actor>', 'approval actor', 'human').action(async (decision: string, runId: string | undefined, command: { readonly by: string }) => print(await approveRun({ configPath: options().config, runId, decision, actor: command.by })))
program.command('authorize <decision> [run-id]').description('Authorize or reject declared external tracking.').option('--by <actor>', 'approval actor', 'human').action(async (decision: string, runId: string | undefined, command: { readonly by: string }) => print(await authorizeRun({ configPath: options().config, runId, decision, actor: command.by })))
