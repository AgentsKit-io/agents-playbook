...
import { approveRun, authorizeRun, benchmarkRuns, cancelRun, cleanTaskArtifacts, createDocBridgeContextProvider, exportEvidenceBundle, loadBenchmarkManifest, loadConfig, loadLatestRun, planRun, readContextSnapshots, reconcileRun, recordBenchmarkObservation, retryRun, startRun, verifyEvidenceBundle, verifyRun } from './index.js'
...
events.command('export [run-id]').description('Export a reconciled COMPLETE run as a signed evidence bundle.').requiredOption('--output <path>', 'bundle output path').requiredOption('--private-key <path>', 'Ed25519 private key path').action(async (runId: string | undefined, command: { readonly output: string; readonly privateKey: string }) => print(await exportEvidenceBundle({ configPath: options().config, runId, outputPath: command.output, privateKeyPath: command.privateKey })))
events.command('verify-bundle <path>').description('Verify an exported signed evidence bundle independently.').action((path: string) => print(verifyEvidenceBundle(path)))
