...
it('reports artifact acceptance without making an incomplete baseline comparable', () => {
  ...
  const manifest = validateBenchmarkManifest({ type: 'agentskit-harness-benchmark-manifest', schemaVersion: 1, suiteId: 'suite', name: 'Fixture', tasks: [{ id: 'task', title: 'Task', acceptanceCriteria: ['criterion'] }], observations: [{ taskId: 'task', status: 'failed', source: 'fixture', recordedAt: '2026-01-01T00:00:00.000Z', durationMs: 200, artifactAcceptanceRate: 0.6667, protocolCompletionRate: 0, evidence: [{ criterion: 'criterion', status: 'passed', source: 'fixture' }] }] })
  const comparison = benchmarkRuns(stateDir, manifest).comparisons[0]
  expect(comparison).toMatchObject({ comparable: false, comparability: 'baseline-incomplete', baselineArtifactAcceptanceRate: 0.6667, baselineProtocolCompletionRate: 0, improvement: { duration: 'unavailable', artifactAcceptanceRate: 0.4999, artifactAcceptance: 'improved', artifactAcceptanceDelta: 0.3333, protocolCompletionRate: null, protocolCompletion: 'improved', protocolCompletionDelta: 1 }, harness: { artifactAcceptanceRate: 1, artifactAcceptanceSampleCount: 1, protocolCompletionRate: 1, protocolCompletionSampleCount: 1 } })
})
