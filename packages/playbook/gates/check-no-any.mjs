...
//
// Config: .quality-gates.json { gates: { "no-any": { allowHatch: "//\\s*allow-any:", maxHatches: 0 } } }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((m = re.exec(line))) {
      ...
      const isTypePosition =
        ...
        /\btype\s+[A-Za-z_$][A-Za-z0-9_$]*(?:\s*<[^>]*>)?\s*=\s*$/.test(before) ||
        /<[^>]*\b[A-Za-z_$][A-Za-z0-9_$]*\s*=\s*$/.test(before) ||
        /\b(?:extends|implements|keyof)\s+$/.test(before) ||
        /^\s*\[\]/.test(after) ||
        /^\s*[|&]/.test(after) ||
        /\bas\s+$/.test(before);
...
async function main() {
  ...
  for (const dir of config.scan) {
    ...
    for await (const file of walk(full, config.ignore)) {
      ...
      for (const h of hits) {
        violations.push({ file: relative(ROOT, file).split(sep).join("/"), ...h });
      }
      hatches += raw.split("\n").filter((line) => config.allowHatch.test(line)).length;
    }
  }
  ...
  if (hatches > config.maxHatches) {
    console.error(`✗ no-any escape-hatch budget exceeded: ${hatches} found, maximum ${config.maxHatches}.`);
    console.error(`  Remove the new hatch or explicitly raise gates.no-any.maxHatches with review.`);
    return 1;
  }
