...
async function loadConfig() {
  ...
  const user = raw?.gates?.secrets ?? {};
  return {
    ...DEFAULT_CONFIG,
    ...user,
    allowFiles: user.allowFiles?.map((value) => new RegExp(value)) ?? DEFAULT_CONFIG.allowFiles,
...
async function main() {
  ...
  for (const dir of config.scan) {
    ...
    if (fromRoot === ".." || fromRoot.startsWith(`..${sep}`) || isAbsolute(fromRoot)) {
      throw new Error(`gates.secrets.scan path escapes the repository: ${dir}`);
    }
  ...
  for (const f of failures) {
    console.error(`✗ ${f.file}:${f.line} — likely ${f.name} (value redacted). Replace with a vault reference; never commit raw secrets.`);
  }
  ...
  console.error(`  Allow file: add a regex source string to .quality-gates.json gates.secrets.allowFiles.`);
  return 1;
}
