...
const manifest = resolve(root, process.env.HARNESS_BENCHMARK_MANIFEST ?? 'benchmarks/harness-phase-10.json')
...
console.log(JSON.stringify(failures.length ? { status: 'failed', criteria: ['benchmark-outcomes'], failures } : { status: 'passed', criteria: ['benchmark-outcomes'], taskCount, comparableTaskCount: report.manifest.comparableTaskCount }))
