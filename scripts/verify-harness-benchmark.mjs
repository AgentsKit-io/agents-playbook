...
const manifest = resolve(root, process.env.HARNESS_BENCHMARK_MANIFEST ?? 'benchmarks/harness-phase-8.json')
...
console.log(JSON.stringify(failures.length ? { status: 'failed', criteria: ['benchmark-comparability'], failures } : { status: 'passed', criteria: ['benchmark-comparability'], taskCount, comparableTaskCount: report.manifest.comparableTaskCount }))
